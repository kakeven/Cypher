  ...
  <div>
    ...
    <section class="card form-card">
      ...
      <form class="form-grid" @submit.prevent="save">
        ...
        <label>A cada quantos períodos<input v-model="form.interval" class="input" type="number" min="1" max="365" required /></label>
    ...
    <section class="recurring-section"><header><h2>Suas recorrências</h2></header><div v-if="loading" class="empty">Carregando…</div><p v-else-if="!recurring.length" class="empty card">Nenhuma recorrência cadastrada.</p><div v-else class="recurring-grid"><article v-for="item in recurring" :key="item.id" class="card recurring-card"><span :class="item.is_active ? 'active' : 'paused'">{{ item.is_active ? 'Ativa' : 'Pausada' }}</span><h3>{{ item.name }}</h3><p>{{ item.type === 'income' ? 'Receita' : 'Despesa' }} de <b>{{ brl(item.amount) }}</b></p><small>{{ item.frequency === 'monthly' ? `Todo dia ${item.day_of_month}` : item.frequency === 'weekly' ? 'Semanal' : item.frequency === 'yearly' ? 'Anual' : 'Diária' }}</small><div><button class="link" @click="generate(item)">Gerar 12 meses</button><button class="link" @click="edit(item)">Editar</button><button class="link" @click="toggle(item)">{{ item.is_active ? 'Pausar' : 'Reativar' }}</button><button class="link danger" @click="remove(item)">Excluir</button></div></article></div></section>
