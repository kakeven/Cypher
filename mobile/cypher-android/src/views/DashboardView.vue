...
const alerts = computed(() => {
  return [
    ...
    ...receivables.value.filter((item) => !item.is_paid && item.remaining_amount > 0).map((item) => ({ label: item.name, detail: `${brl(item.remaining_amount)} pendentes`, type: 'positive' })),
  ]
})
    ...
    <template v-else-if="data">
      <section class="dashboard-summary" aria-label="Resumo financeiro">
        <article class="balance-card">
          ...
          <small :class="data.income >= data.net_expense ? 'positive' : 'negative'">{{ data.income >= data.net_expense ? 'Saldo positivo no mês' : 'Despesas próprias acima das receitas' }}</small>
        </article>
        <article class="month-card">
          ...
          <div class="quick-stats">
            ...
            <div><span>Cartão pendente</span><b class="negative">{{ brl(data.card_pending) }}</b></div>
      ...
      <template v-else>
        <section class="flow-card">
          <header class="flow-header"><div><h2>Fluxo financeiro</h2><p>Receitas e despesas próprias ao longo do ano.</p></div><div class="flow-legend"><span class="income"><i />Receitas</span><span class="expense"><i />Despesas próprias</span></div></header>
...
.quick-stats { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 8px; margin-top: 12px; }
